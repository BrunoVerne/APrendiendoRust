...
fn levantar_server(addr: &str) {
    ...
    let store: Arc<Mutex<HashMap<String, Option<String>>>> =
        ...
        for stream in listener.incoming() {
            match stream {
                Ok(s) => {
                    ...
                    thread::spawn(move || {
                        server::handle_connection::handle_connection(s, store)
                    });
                }
                Err(_) => break,
            }
...
fn test_not_found_via_tcp() {
    ...
    assert_eq!(enviar("127.0.0.1:18002", "get noexiste"), "ERROR: NOT FOUND");
}

#[test]
fn test_extra_argument_via_tcp() {
    levantar_server("127.0.0.1:18003");
    assert_eq!(enviar("127.0.0.1:18003", "set a b c"), "ERROR: EXTRA ARGUMENT");
}

#[test]
fn test_unknown_command_via_tcp() {
    levantar_server("127.0.0.1:18004");
    assert_eq!(enviar("127.0.0.1:18004", "delete a"), "ERROR: UNKNOWN COMMAND");
}
